...
```{r}
sanitation_MDG_2020_2022_not_safely_managed <-sanitation |>
...
```{r}
sanitation_MDG_2020_2022_not_safely_managed |> 
  count(year)
```
...
```{r}
ggplot(data = sanitation_MDG_2020_2022_not_safely_managed,
...
1.  Look at the plot that you created. What do you notice about the order of the bars / order of the legend? **The order of the stacked bars and legend don't follow the JMP sanitation ladder, which is confusing for the viewer.**
2.  What would you want to change? **I would change the order so the bars followed the sanitation ladder (no facilities/blue should be at the bottom).**
3.  Why did we remove "safely managed sanitation services" from the data set in Task 3? **It's probably very low in many countries and there could be doublecounting/overlap with basic?**
...
2.  What do you observe when you look at the code and plot?
    ...
    **scales = "free_y" seems to allow the y-axis for the plots to change depending on the data range. Also with geom_histogram, you don't need to set the y-axis since it defaults to count.**
