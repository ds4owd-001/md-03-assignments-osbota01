...
```{r}
penguins |> 
  filter(sex == "male", island %in% c("Dream", "Torgersen") )
```
...
Starting with `penguins`, group the data by species, then create a summary table containing the maximum and minimum length of flippers (call the columns flip_max and flip_min). How will you handle NA values? **Use na.rm = TRUE to indicate missing values should be removed**
...
Starting with `penguins`, group the data by species and year, then create a summary table containing the mean bill depth (call this bill_depth_mean), the mean bill length (call this bill_length_mean), and the count for each group. How will you handle NA values? **Filter out NA values using filter (!is.na... for bill length and depth variables.**

**I also tried using the na.rm = TRUE option instead of filter and it seemed to work but a few of the counts are slightly different.**
